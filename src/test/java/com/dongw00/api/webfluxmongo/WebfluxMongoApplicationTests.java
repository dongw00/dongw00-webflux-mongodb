package com.dongw00.api.webfluxmongo;
...
import com.dongw00.api.webfluxmongo.config.CustomException;
...
class WebfluxMongoApplicationTests {
    ...
    void main() {
        ...
        assertThat(names, is(equalTo(Arrays.asList("1", "2", "3"))));
    }
    ...
    void fromStreamTest() {
        ...
        Flux<String> flux = Flux.fromStream(Stream.of("1", "2", "3")).log();
        flux.subscribe(list::add);
        assertThat(list, is(equalTo(Arrays.asList("1", "2", "3"))));
    }
    ...
    void createTest() {
        ...
        Flux<Integer> flux = Flux.create((FluxSink<Integer> sink) -> {
            sink.onRequest(req -> {
                for (int i = 1; i < req + 3; i++) { // subscriber가 요청한 것 보다 3개더 발생
                    sink.next(i);
                }
            });
        });
